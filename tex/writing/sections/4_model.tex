...
To ensure \todo{...}, I introduce a dynamic panel specification,
exploiting the time series observations available in this data.
...
\begin{align}
    y_{i,t} &= \alpha_i + \beta_0
                ...
                + \beta_1 k_{i,t}
                + \beta_2 l_{i,t}
                + \beta_3 \sum_{j\in-i} w_{ijt}z_{jt}
                + \varepsilon_{it}
    \\
    \Delta y_{it} &= \beta_0
                + \sum_{k=1}^p \gamma_k \Delta y_{i,t-k}
                + \beta_1 \Delta k_{i,t}
                + \beta_2 \Delta l_{i,t}
                + \beta_3 \Delta \sum_{j\in-i} w_{ijt}z_{jt}
    ...
    \right]
\end{align}
\todo{Why do we have a constant after first differencing?}

I return to a first-stage production function with regressand $y_{it}$.
I continue to use the scalar weighted distance TFP metric
as the primary regressor of interest $z_{jt}$.
\par
I aim to include lags of $ln(y_{it})$ as regressors,
using this AR structure to strip out as much endogenous variation of the current observation
as possible.
...
As I am using lags as regressors, I lose strict endogeneity and instead can only claim
$mathbb E[\varepsilon_{it}|x_{i\tau}]=0\ \forall t\geq \tau$.
As I result, when I eliminate firm fixed effects via first-differencing, 
my resulting $\Delta x_{it}$ is correlated with the differenced error time $\Delta\varepsilon_{it}$.
\par
To overcome this, I introduce instrumental variables using deeper lags
as proposed by \citep{Anderson1981},
namely $y_{i,t-p-1}, y_{i,t-p-2}, y_{i,t-p-3}$ for GVA
and  $k_{i,t-p-1}, k_{t-p-2}$ for capital.
I use multiple lags as instrument to increase identification power.
As the Fame dataset is rich on the time dimension, namely I have up to 19 lags at most for a firm,
with a mean of 7 lags, I can safely do this for any structure up to AR(4) given 3 lags.
Results show that the coefficient after 4 lags becomes insignificant,
with theory suggesting that should continue to decline with additional lags.
An ideal approach would select the auto-regressive order via information criteria tests.
I assume that contemperaneous changes in peer TFP $z_{jt}$ are exogenous,
though this new specification now ensures that only the component of $z_{jt}$
that does not correlate with lags of output is measured.
\todo{Even so, this is still a strong assumption.
I should introduce a control (basically a diff-in-diff?) of peers from the same industry
...
\wraptable
    ...
    {\todo{
        We can add more lags, but 2 lags already suggests AR(1) structure.
        Interestingly, my laptop crashed executing AR(3)
    }}
