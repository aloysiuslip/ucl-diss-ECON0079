...
This analysis rests on the plausibility of the group peer effect,
$\sum_{j\in-i} w_{ijt}z_{jt}$ as an exogenous regressor.
...
however, my results show that AR(1) is the true structure.
...
\todo{(IC test results)}
\par
This dynamic panel approach requires a valid choice of instruments.
Since I take first differences of my log aggregates to strip out firm fixed effects,
I strongly expect $\Delta ln(y_{it})$, my regressand,
to be correlated with $\Delta ln(y_{it})$, my AR regressor,
meaning this first-order lag should be treated as a pre-determined regressor.
...
\section{Dynamic panel with additional controls}
\label{ss:model-dyn-controls}