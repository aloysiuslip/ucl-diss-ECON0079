...
and identification approach for models in Tables \ref{tab:lmm-pc8} and \ref{tab:lmm-pc8}.
\par
I use the instruments $\mathbf W^2\mathbf x_t$ and other second-order variants
for this section's group models \todo{Table VI, models (5-8)}
and $\mathbf W^2\mathbf x_t, \mathbf W^3\mathbf x_t, \mathbf W^4$
as feasible instruments for this section's network models in \tablerefp{tab:dd}.
...
assuming the full vector of error orthogonality implying regressor exogeneity.
...
of 3 independent matrices $\theta_k \mathbf W_{gk}$.
...
do not affect the variation attributable to the $\mathbf W_{g1}$-lagged variables
as the regressors with $\mathbf W_{g2}, \mathbf W_{g3}$ are orthogonal.
...
\par
I estimate this model in panel IV reduced-form only.
\begin{align}
    \Delta y_{i,t} &= \Delta\gamma_t
                    + \beta_1 \Delta k_{i,t}
                    + \beta_2 \Delta l_{i,t}
...
\\
This distance-decay specification revises down the estimate from peers to an elasticity of 0.168,
lower than the results given by the LMM specification. This is likely due to \todo{...}
\\
In addition, I introduce controls \todo{...}
\todo{I run a variant where firms which share a location, meaning distance is mechanically 0, are excluded}.