...
and identification approach for the group models in Table \ref{tab:lmm_pc8}.
\par
I use the instruments $\mathbf W^2\mathbf x_t$ and other second-order variants
for this section's group models \ref{tab:lmm_pc8}
...
\noindent
Results for Model (1.) and (2.) are given in Table \ref{tab:lmm_pc8}.

\wraptable
        {LMM: 1 peer regressor with group weights, error orthogonality model vs instrumented}
        {../../model/output/results_1a_lmm_pc8}
        {
...
I apply the $\mathbf W_{g1}, \mathbf W_{g2}, \mathbf W_{g3}$ concentric doughnut group transformations
...
however the use of multiple orthogonal $\mathbf W_g$ matrices
opens up 
Algebraically, multiple spatial autoregressors do not change
the peer effects endogeneity and and valid instrument problem,
...
which here is guaranteed by the mutual orthogonality of $\mathbf W_g$,
then there is no risk of multicollinearity.
\begin{align}
    \mathbf y_t &= \boldsymbol \alpha
                    + \gamma_t \boldsymbol \iota
                    + \beta_0 \boldsymbol \iota
...
Either any combination of higher-order $\mathbf W_{gs}$ group transformations
...
Coefficients in Table \ref{tab:lmm-3r} show that
$\hat\delta_1$ estimates match those of $\hat\delta_0$ in Table \ref{tab:lmm_pc8},
this is an intuitive result due to the orthogonality of $\mathbf W_{g}\mathbf x$ regressors.
...
\begin{table}[hbtp]
    ...
    \begin{threeparttable}
        ...
        \label{tab:lmm-3r}
        \input{../../model/output/results_1b_lmm_3r}
        \begin{tablenotes}
