...
with the dot product against the vector of their respective spillover coefficients $\theta_0$:
\begin{align}
...
The objective is to estimate the vector of structural coefficients $\theta_0$
...
\begin{align}
    ...
    \label{eq:static-main-reduced}
            &= \alpha_i + \gamma_t + \beta_0 + \mathbf z_t^{spill}\theta_0 + \varepsilon_{i,t}
    \\
...
\par
It is important to note that I do not impose restrictions on $\theta_0$.
...
Through this 3-factor production model, it is hard to link each parameter in $\theta_0$
...
A restriction on logged production \eqref{eq:static-main-gva} collectively on $\beta_1$, $\beta_2$, and $\theta_0$
...
Capital and labour in every firm $i,j$ are assumed to be exogenous in this static model
...
The inversion is permissible because I can expect a restriction to apply, ruling out iteratively increasing shocks\footnote{
    $\delta_0>1$ would imply an exogenous TFP shock to firm $k$ would
            ...
            & \begin{aligned}
                = \frac{\gamma_t + \beta_0}{1-\delta_0}\boldsymbol \iota
                    ...
                    + \sum_{k=0}^\infty \delta_0^k \mathbf W^k
                        \boldsymbol \varepsilon_{t}
                    \\
                    &+ (\xi_0  -\delta_0\beta_1 + \delta_0\beta_1)\sum_{k=0}^\infty \delta_0^k \mathbf W^{k+1} \mathbf k_t
                    + (\zeta_0-\delta_0\beta_2 + \delta_0\beta_2)\sum_{k=0}^\infty \delta_0^k \mathbf W^{k+1} \mathbf l_t
            \end{aligned}
    \\
...
This hinders identification of $\delta_0$.
\par
...
\vspace{5cm}
\todo{First set of assumed-exogenous eestimations}
\vspace{5cm}

\todo{separate this out into 2 tables: single model, and 3 rings model}
\par
The second model instruments these groups via $\mathbf W^2\mathbf x$,
...
\vspace{5cm}
\todo{Instrument equations and regression table}
\vspace{5cm}
